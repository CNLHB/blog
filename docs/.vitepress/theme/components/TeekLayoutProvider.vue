...
const watchRuntimeAndRibbon = async (layout: string, style: string) => {
  ...
  if ((isHome && isBlog && style !== "blog-body") || (isDoc && teekConfig.value.pageStyle))
    startRibbon();
  else stopRibbon();
};

watch(frontmatter, async (newVal) => watchRuntimeAndRibbon(newVal.layout, currentStyle.value), {
  immediate: true
});
